...
```{r cadmium-summary }
# Summary Statistics
summary(meuse.sf$zinc) 
summary(log(meuse.sf$zinc))
```

```{r plot-stem-leaf }
# Steam and leaf plot
stem(meuse.sf$zinc) 
stem(log(meuse.sf$zinc))
```

```{r plot-hist }
# Histogram and Q-Q Plots
par(mfrow=c(1,2)) # arranged in 1 row and 2 columns
hist(meuse.sf$zinc, n=20, main = "Histogram of Zinc (ppm)")
#curve(dnorm(x, mean=mean(meuse.sf$zinc), sd=sd(meuse.sf$zinc)), add=TRUE)
abline(v=mean(meuse.sf$zinc), col="red")-
#text(mean(meuse.sf$zinc),0.04,"Mean", col = "red", adj = c(-0.1, -15))
abline(v=median(meuse.sf$zinc), col="blue")
#text(median(meuse.sf$zinc),0.04,"Median", col = "blue", adj = c(-0.1, -18))

hist(log(meuse.sf$zinc), n=20, main = "Histogram of Log-Zinc (ppm)")
#curve(dnorm(x, mean=mean(log(meuse.sf$zinc)), sd=sd(log(meuse.sf$zinc))), add=TRUE)
abline(v=mean(log(meuse.sf$zinc)), col="red")
#text(mean(log(meuse.sf$zinc)),0.04, "Mean", col = "red", adj = c(-0.1, -15))
abline(v=median(log(meuse.sf$zinc)), col="blue", main = NULL, sub = NULL)
#text(median(log(meuse.sf$zinc)),0.04, "Median", col = "blue", adj = c(-0.1, -17))
```
Mean (red) and Median (blue) have been highlighted.

```{r plot-box }
par(mfrow=c(1,2))
boxplot(meuse.sf$zinc, main = "Boxplot of Zinc (ppm)")
boxplot(log(meuse.sf$zinc), main = "Boxplot of Log-Zinc (ppm)")
```

```{r plot-qq }
par(mfrow=c(1,2))
qqnorm(meuse$zinc,  main = "Q-Q Plot of Zinc  (ppm)")
qqnorm(log(meuse$zinc),  main = "Q-Q Plot of Log-Zinc (ppm)")
...
Generally we want to _interpolate_ data because **we either do not have the resources _(money and time)_ or its simply not feasible** to collect data everywhere. 
...
- **Question 7.**  Use the following code chunk to perform the various analyses required for a regression modeling with distance to river as an independent variable.
...
plot(log(zinc)~sqrt(dist), as.data.frame(meuse.sf),
     main="Scatterplot of log(Zn) vs. Squ. dist.")
abline(lm(log(zinc)~sqrt(dist), meuse.sf))#, col="blue")
```
...
plot(meuse.grid.sf['lzn.fit'], #"lzn.fit", #sp.layout = meuse.lt,
     main = "Log(Zinc) - ppm: Regression model values")
```

```{r plot-std-fit }
# Plot the Standard Error of fit
plot(meuse.grid.sf["se.fit"], #, sp.layout = meuse.lt,
     main = "Log(Zinc) - ppm: Regression model Standard Error of fit")
```
...
meuse.grid.sf$idwp05 = idw(log(zinc)  ~ 1, meuse.sf, meuse.grid.sf, idp = 0.5)$var1.pred
#meuse.grid.sf$idwp1 = idw(log(zinc)  ~ 1, meuse.sf, meuse.grid.sf, idp = 1)$var1.pred
meuse.grid.sf$idwp25 = idw(log(zinc)  ~ 1, meuse.sf, meuse.grid.sf, idp = 2.5)$var1.pred
meuse.grid.sf$idwp5 = idw(log(zinc)  ~ 1, meuse.sf, meuse.grid.sf, idp = 5)$var1.pred
meuse.grid.sf$idwp10 = idw(log(zinc)  ~ 1, meuse.sf, meuse.grid.sf, idp = 10)$var1.pred
...
We are now firmly in the realm of Trend Surface Analysis. What we mean by a **_trend surface_** is: a variable (say $z$, zinc deposits, elevation, etc.) can be expressed some smooth function of coordinates ($x_i, y_i$). A polynomial function of the coordinates similar to:
...
In other words we; _predict_ values at upsampled locations (the `meuse grid`) with a 1st-, 2nd- or 3rd-order polynomial. The higher the degree of the polynomial (1st, 2nd, 3rd, etc.) the more the surface matches the original data. We should however take care.
...
meuse.grid.sf$tr1 = krige(log(zinc) ~ 1, meuse.sf, meuse.grid.sf, degree = 1)$var1.pred
meuse.grid.sf$tr2 = krige(log(zinc) ~ 1, meuse.sf, meuse.grid.sf, degree = 2)$var1.pred
meuse.grid.sf$tr3 = krige(log(zinc) ~ 1, meuse.sf, meuse.grid.sf, degree = 3)$var1.pred
...
plot(meuse.grid.sf[c("tr1", "tr2", "tr3")], pch=15,
     main = "Log(zinc) - ppm Trend Surface Interpolation") #main="IDW") #"idwp1",
...
One of the principles of Spatial Data Science is; **data is spatially correlated**. Patterns and particularly residuals (errors) have a spatial structure / are connected.
...
hscat(log(zinc)~1, meuse.sf, (0:9)*100)
```
...
###################################################
plot(variogram(log(zinc) ~ 1, meuse.sf))
```
...
###################################################
plot(variogram(log(zinc) ~ 1, meuse.sf, cutoff = 1000, width = 50))
```
...
###################################################
variogram(log(zinc) ~ 1, meuse.sf, boundaries = c(0,50,100,seq(250,1500,250)))
```
...
<!-- # ################################################### -->
<!-- # fit.variogram.reml(log(zinc)~1, meuse.sf, model=vgm(0.6, "Sph", 800, 0.06)) -->
<!-- # ``` -->
...
###################################################
v.dir <- variogram(log(zinc)~1, meuse.sf, alpha=(0:3)*45) 
...
###################################################
plot(variogram(log(zinc)~1, meuse.sf, map=TRUE, cutoff=1000, width=100))
```
...
<div class="alert alert-danger"> <strong>STOP HERE!</strong> </div>

<div class="alert alert-warning"> <strong>QUESTION!</strong> </div>

- **Question 10.**  Use the next command to perform Kriging interpolation and comment on the output, plot and results.
...
###################################################
lz.sk <- krige(log(zinc)~1, meuse.sf, meuse.grid.sf, v.fit, beta = 5.9)
lz.ok <- krige(log(zinc)~1, meuse.sf, meuse.grid.sf, v.fit)

par(mfrow=c(1,2)) # arranged in 1 row and 2 columns
plot(lz.sk, pch=15, nbreaks = 64, main ="SK prediction [log10(Zn ppm)]")
plot(lz.ok, pch=15, nbreaks = 64, main = "OK prediction [log10(Zn ppm)]")
par(mfrow=c(1,1)) # Reset to default plotting of 1 figure per page
```
...
```{r cross }
#library(sp)
idw.cv.a <- krige.cv(log10(zinc) ~ 1, meuse.sf, nmax = 7, nfold=5, set = list(idp = 2.5))
ols.cv.a <- krige.cv(log10(zinc) ~ 1, meuse.sf, degree = 2, nmax = 40, nfold=5)
#ok.cv.a <- krige.cv(log10(zinc) ~ 1, locations = meuse.sf, model = v.fit)
ok.cv.a  <- krige.cv(log(zinc)~1, meuse.sf, v.fit, nmax = 40, nfold=5)
```
...
- **Question 13.** Explain what we mean with $N$-fold validation and comment on the other validation method offered by the `krige.cv()` function of the `gstat` package.
...
```{r cv-correlation}
par(mfrow=c(1,3))
plot(var1.pred ~ observed, idw.cv.a, main="IDW", ylab="IDW pred.")
plot(var1.pred ~ observed, ols.cv.a, main="OLS", ylab="OLS pred.")
...
```{r cv-bubble-idw }
plot(idw.cv.a["residual"], main = "log(zinc): IDW 5-fold CV residuals", #reset = FALSE,
     cex=4*idw.cv.a$residual/max(idw.cv.a$residual), reset = FALSE)
plot(meuse.riv.sf, add = TRUE)
```

```{r cv-bubble-ols }
plot(ols.cv.a["residual"], main = "log(zinc): 2nd-order OLS 5-fold CV residuals",
          cex=4*ols.cv.a$residual/max(ols.cv.a$residual), reset = FALSE)
plot(meuse.riv.sf, add = TRUE)
```
```{r cv-bubble-ok }
plot(ok.cv.a["residual"], main = "log(zinc): OK 5-fold CV residuals",
          cex=4*ok.cv.a$residual/max(ok.cv.a$residual), reset = FALSE)
plot(meuse.riv.sf, add = TRUE)
```
```{r }
boxplot(idw.cv.a$residual, ols.cv.a$residual, ok.cv.a$residual, main="IDW, OLS, OK")
```
...
- **Question 16.** Having gone through a variety of interpolation methods along with both a qualitative and quantitative evaluation; which would you choose? Motivate your choice.
