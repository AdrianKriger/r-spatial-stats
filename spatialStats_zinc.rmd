...
____
<div class="alert alert-success"> <strong>CONCEPT CHECK:</strong> Thus is a concept we will come back to when we introduce spatial correlation.
</div>
...
- **Question 9.**  Use the next set of commands to estimate, model and fit the variogram and comment on the output, plots and results.
...
hscat(log(zinc)~1, meuse.sf, (0:8)*100)
```
...
par(mfrow=c(1,2)) # arranged in 1 row and 2 columns
plot(lz.sk['var1.pred'], pch=15, nbreaks = 64, main ="Simple Kriging [log10(Zn ppm)]",
     reset = FALSE)
plot(lz.ok['var1.pred'], pch=15, nbreaks = 64, main = "Ordinary Kriging [log10(Zn ppm)]")
par(mfrow=c(1,1)) # Reset to default plotting of 1 figure per page
```
